...
class dense_assigner:
    ...
    def assign(self):
        if self.new_bias is None:
            return [self.new_kernel]
        else:
            return [self.new_kernel, self.new_bias]
...
class spectral_assigner:
    ...
    def assign(self):
        if self.new_bias is None:
            return [self.new_base, self.new_diag_end, self.new_diag_start]
        else:
            return [self.new_base, self.new_diag_end, self.new_bias, self.new_diag_start]
...
def dense_weights_distiller(to_prune: Dense, cut_off: float):
    ...
    weights = to_prune.get_weights()
    if len(weights) == 2:
        new_bias = weights[1][out_cond]
    else:
        new_bias = None

    new_kernel = weights[0][:, out_cond]
...
def spectral_weights_distiller(to_prune: Spectral, cut_off: float, link_mode: str = 'or'):
    ...
    if len(weights) == 4:
        new_bias = weights[2][out_cond]
        new_diag_start = weights[3][in_cond, :]
    else:
        new_bias = None
        new_diag_start = weights[2][in_cond, :]

    new_base = weights[0][:, out_cond]
    new_base = new_base[in_cond, :]
    return spectral_assigner(new_shape=np.sum(out_cond*1),
                             new_base=new_base,
                             new_diag_end=weights[1][:, out_cond],
...
def spectral_pruning(model: Model, percentile: int):
    ...
    for lay in index_list:
        if lay["layer"] == 'Spectral':
            ...
            continue
            # raise ValueError("Invalid layer to be pruned, check the list.\n "
            #                  "Valid types: Spectral, Dense")
    ...
    for assigner in new_weights:
        ...
        pruned.set_weights(assigner.assign())
